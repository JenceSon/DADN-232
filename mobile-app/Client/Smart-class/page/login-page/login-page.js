...
export function Login() {
    ...
    function SignUpModal() {
        return (
            <>
                <CusModal title={"Sign Up"} isVisible={signUpModal}
                          setVisibleState={() => setSignUpModal(preState => !preState)}
                          handleSubmit={handleSignUp}
                >
                    <View className={"my-2 flex flex-col justify-center gap-5"} style={{width: "100%"}}>
                        <Input label={"Id"} placeholder={"2110913"} className={"flex-auto w-64"}
                               value={signUpInfo["id"]}
                               onChangeText={(id) => signUpInfo.current = {...signUpInfo.current, id}}/>
                        <Input label={"Email"} placeholder={"khuongduy429@gmail.com"} className={"flex-auto w-64"}
                               onChangeText={(Email) => signUpInfo.current = {...signUpInfo.current, Email}}/>
                        <Input label={"Password"} placeholder={""} className={"flex-auto w-64"}
                               onChangeText={(Pw) => signUpInfo.current = {...signUpInfo.current, Pw}}/>
                        <RadioButton.Group
